import { CourseData, Phase, phaseStart } from './course-data';
import { HorseParameters } from './horse-type';
import { GroundCondition } from './race-parameters';
import type { RaceState } from './race-solver';
...
export class GameHpPolicy {
  ...
  getLastSpurtPair(state: RaceState, maxSpeed: number, baseTargetSpeed2: number) {
    const maxDist = this.distance - phaseStart(this.distance, 2);

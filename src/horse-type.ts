...
export function assertIsStrategy(strategy: number): asserts strategy is Strategy {
  assert(strategy >= 0); // Strategy validation simplified
}

export function strategyMatches(s1: Strategy, s2: Strategy) {
  return s1 == s2 || (s1 == Strategy.Front && s2 == Strategy.Escape) || (s1 == Strategy.Escape && s2 == Strategy.Front);
}

import { Conditions, immediate, noopRandom, Operator, random } from './activation-conditions';
import { ActivationSamplePolicy, ImmediatePolicy } from './activation-sample-policy';
import { getParser } from './condition-parser';
import { CourseData, courseSpeedModifier, DistanceType, getCourse, phaseEnd, phaseStart } from './course-data';
import { Aptitude, HorseParameters, Strategy } from './horse-type';
import { GameHpPolicy, NoopHpPolicy } from './hp-policy';
import { Grade, GroundCondition, Mood, RaceParameters, Season, Time, Weather } from './race-parameters';
import {
  DynamicCondition,
  PendingSkill,
  Perspective,
...
interface ConditionParser {
  parse: (tokens: Iterator<unknown>) => Operator;
  tokenize: (condition: string) => Iterator<unknown>;
}
...
export const AsitameStrategyDistanceCoefficient = Object.freeze([
  ...
  [0, 1.0, 0.7, 0.75, 0.7, 1.0], // short (front, pace, late, end, escape)
...
export function calcAsitameApproximateModifier(power: number, strategy: Strategy, distance: DistanceType) {
  ...
  return Object.freeze({
    speed: Math.max(
      baseStats.speed * raceCourseModifier +
        (GroundSpeedModifier[course.surface] !== null && GroundSpeedModifier[course.surface] !== undefined
          ? (GroundSpeedModifier[course.surface] as number[])[ground]
          : -1),
      1,
    ),
    ...
    power: Math.max(
      baseStats.power +
        (GroundPowerModifier[course.surface] !== null && GroundPowerModifier[course.surface] !== undefined
          ? (GroundPowerModifier[course.surface] as number[])[ground]
          : -1),
      1,
    ),
    guts: baseStats.guts,
...
function buildSkillEffects(skill, perspective: Perspective) {
  ...
  return skill.effects.reduce((acc, ef) => {
    if (isTarget(perspective, ef.target) && validSkillTypes.includes(ef.type)) {
      acc.push({
        type: ef.type,
...
export function buildSkillData(
  ...
  const _extra = Object.assign({ skillId }, raceParams);
  ...
  for (let i = 0; i < alternatives.length; ++i) {
    ...
    if (skill.precondition) {
      ...
      const preRegions = pre.apply(wholeCourse, course, horse, _extra)[0];
    ...
    const [regions, extraCondition] = op.apply(full, course, horse, _extra);
    if (regions.length == 0) {
      continue;
    }
    if (triggers.length > 0 && !/is_activate_other_skill_detail|is_used_skill_id/.test(skill.condition)) {
      ...
      // !!! FIXME this is actually bugged for NY Ace unique since she'll get both effects if she uses escape.
      continue;
    }
...
export const conditionsWithActivateCountsAsRandom = Object.freeze(
  Object.assign({}, Conditions, {
    activate_count_all: random({
      filterGte(regions: RegionList, n: number, course: CourseData, _1: HorseParameters, _extra: RaceParameters) {
        ...
        /*if (_extra.skillId == '110151' || _extra.skillId == '910151') {
				const rl = new RegionList();
      ...
      filterLte(_regions: RegionList, _n: number, _course: CourseData, _1: HorseParameters, _extra: RaceParameters) {
    ...
    activate_count_end_after: random({
      filterGte(regions: RegionList, _0: number, course: CourseData, _1: HorseParameters, _extra: RaceParameters) {
        const bounds = new Region(phaseStart(course.distance, 2), phaseEnd(course.distance, 3));
        return regions.rmap((r) => r.intersect(bounds));
      },
    }),
    ...
    activate_count_middle: random({
      filterGte(regions: RegionList, n: number, course: CourseData, _1: HorseParameters, _extra: RaceParameters) {
        const start = phaseStart(course.distance, 1),
          end = phaseEnd(course.distance, 1);
    ...
    activate_count_start: immediate({
      // for 地固め
      filterGte(regions: RegionList, _0: number, course: CourseData, _1: HorseParameters, _extra: RaceParameters) {
        const bounds = new Region(phaseStart(course.distance, 0), phaseEnd(course.distance, 0));
        return regions.rmap((r) => r.intersect(bounds));
      },
    }),
  }),
...
export class RaceSolverBuilder {
  ...
  constructor(readonly nsamples: number) {
    this._course = new Object() as CourseData;
    this._raceParams = {
      mood: 2,
    ...
    this._horse = new Object() as HorseDesc;
    this._pacer = null;
    this._pacerSkills = [];
    ...
    this._onSkillActivate = () => {};
    this._onSkillDeactivate = () => {};
  }
  ...
  _isNige() {
    if (typeof this._horse.strategy == 'string') {
      return this._horse.strategy.toUpperCase() == 'FRONT' || this._horse.strategy.toUpperCase() == 'ESCAPE';
    } else {
      return this._horse.strategy == Strategy.Front || this._horse.strategy == Strategy.Escape;
    }
  }
  ...
  useDefaultPacer(openingLegAccel: boolean = true) {
    ...
    this._pacer = Object.assign({}, this._horse, { strategy: 'Front' });
    if (openingLegAccel) {
  ...
  withAsiwotameru() {
    ...
    this._extraSkillHooks.push((skilldata, horse, course) => {
      ...
      if (power > 1200) {
        ...
        spurtStart.push(new Region(phaseStart(course.distance, 2), course.distance));
        skilldata.push({
          skillId: 'asitame',
          perspective: Perspective.Self,
          rarity: SkillRarity.White,
          ...
          effects: [
            {
              ...
              modifier: calcAsitameApproximateModifier(power, horse.strategy, course.distanceType),
            },
          ],
        });
      }
  ...
  *build() {
    ...
    const pacerHorse = this._pacer
      ? buildAdjustedStats(
          buildBaseStats(this._pacer, this._raceParams.mood),
          this._course,

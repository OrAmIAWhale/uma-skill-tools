...
export const enum ThresholdStat {
  Speed = 1,
  Stamina,
  Power,
  Guts,
...
export function courseSpeedModifier(
  ...
  const statvalues = [0, stats.speed, stats.stamina, stats.power, stats.guts, stats.wisdom].map((x) =>
    Math.min(x, 901),
  );
  return (
    1 +

...
interface Token<T, U> {
  ...
  nud(_state: ParserState<T, U>): Node<T, U>;
}
...
export function getParser<ConditionT = Condition, OperatorT = Operator>(
  ...
  operators: {
    and: new (_left: OperatorT, right: OperatorT) => OperatorT;
    or: new (_left: OperatorT, right: OperatorT) => OperatorT;
    ...
    and: AndOperator as unknown as new (_left: OperatorT, right: OperatorT) => OperatorT, // this is really stupid
    or: OrOperator as unknown as new (_left: OperatorT, right: OperatorT) => OperatorT,
  ...
  const Eof = Object.freeze({
    ...
    nud: (_state: ParserState<ConditionT, OperatorT>): Node<ConditionT, OperatorT> => {
      throw new ParseError('unexpected eof');
    },
  });
  ...
  class Identifier implements Token<ConditionT, OperatorT> {
    ...
    led(_state: ParserState<ConditionT, OperatorT>, _left: Node<ConditionT, OperatorT>): Node<ConditionT, OperatorT> {
  ...
  class CmpOp {
    ...
    led(_state: ParserState<ConditionT, OperatorT>, _left: Node<ConditionT, OperatorT>) {
      if (_left.type != NodeType.Cond) throw new ParseError('expected condition on _left hand side of comparison');
      const right = expression(_state, this.lbp);
      if (right.type != NodeType.Int) throw new ParseError('expected number on right hand side of comparison');
      return {
        type: NodeType.Op,
        op: new this.opclass(_left.cond, right.value),
      } as Node<ConditionT, OperatorT>;
    }
  ...
  class LogicalOp {
    constructor(
      readonly lbp: number,
      readonly opclass: new (_left: OperatorT, right: OperatorT) => OperatorT,
    ) {}

    led(_state: ParserState<ConditionT, OperatorT>, _left: Node<ConditionT, OperatorT>) {
      if (_left.type != NodeType.Op) throw new ParseError('expected comparison on _left hand side of operator');
      const right = expression(_state, this.lbp);
      if (right.type != NodeType.Op) throw new ParseError('expected comparison on right hand side of operator');
      return {
        type: NodeType.Op,
        op: new this.opclass(_left.op, right.op),
      } as Node<ConditionT, OperatorT>;
    }
  ...
  function parseAny(tokens: Iterator<Token<ConditionT, OperatorT>, Token<ConditionT, OperatorT>>) {
    const _state = { current: Eof, next: tokens.next().value, tokens: tokens };
    return expression(_state, 0);
  }
  ...
  function expression(_state: ParserState<ConditionT, OperatorT>, rbp: number) {
    _state.current = _state.next;
    _state.next = _state.tokens.next().value;
    var _left = _state.current.nud(_state);
    while (rbp < _state.next.lbp) {
      _state.current = _state.next;
      _state.next = _state.tokens.next().value;
      _left = _state.current.led(_state, _left);
    }
    return _left;
  }

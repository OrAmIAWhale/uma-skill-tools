...
import { CourseData, isSortedByStart, Phase, phaseStart } from './course-data';
import { HorseParameters, Strategy, strategyMatches } from './horse-type';
import type { HpPolicy } from './hp-policy';
...
if (typeof CC_GLOBAL == 'undefined') globalThis.CC_GLOBAL = false;
...
function baseTargetSpeed(horse: HorseParameters, course: CourseData, phase: Phase) {
  // Defensive check to ensure valid strategy index
  const strategyCoefficients = Speed.StrategyPhaseCoefficient[horse.strategy];
  if (!strategyCoefficients) {
    throw new Error(`Invalid horse strategy ${horse.strategy}: not found in StrategyPhaseCoefficient array`);
  }
  if (phase < 0 || phase >= strategyCoefficients.length) {
    throw new Error(`Invalid phase ${phase}: out of bounds for strategy ${horse.strategy}`);
  }

  return (
    baseSpeed(course) * strategyCoefficients[phase] +
    +(phase == 2) * Math.sqrt(500.0 * horse.speed) * Speed.DistanceProficiencyModifier[horse.distanceAptitude] * 0.002
  );
}
...
export const Acceleration = {
  StrategyPhaseCoefficient: Object.freeze(
    [[], [1.0, 1.0, 0.996], [0.985, 1.0, 0.996], [0.975, 1.0, 1.0], [0.945, 1.0, 0.997], [1.17, 0.94, 0.956]].map((a) =>
      Object.freeze(a),
    ),
  ),
...
export const PositionKeepBaseMinimumThreshold = Object.freeze([0, 0, 3.0, 6.5, 7.5]);
export const PositionKeepBaseMaximumThreshold = Object.freeze([0, 0, 5.0, 7.0, 8.0]);

export function positionKeepCourseFactor(distance: number) {
  return 0.0008 * (distance - 1000) + 1.0;
}

export function positionKeepMinThreshold(strategy: Strategy, distance: number) {
  // pace minimum threshold is a constant 3.0 independent of the course factor for some reason
  return (
    PositionKeepBaseMinimumThreshold[strategy] * (strategy == Strategy.Pace ? 1.0 : positionKeepCourseFactor(distance))
  );
}

export function positionKeepMaxThreshold(strategy: Strategy, distance: number) {
  return PositionKeepBaseMaximumThreshold[strategy] * positionKeepCourseFactor(distance);
}
...
export class RaceSolver {
  ...
  constructor(params: {
    ...
    this.phase = 0;
    this.nextPhaseTransition = phaseStart(this.course.distance, 1);
    ...
    this.posKeepMinThreshold = positionKeepMinThreshold(this.horse.strategy, this.course.distance);
    this.posKeepMaxThreshold = positionKeepMaxThreshold(this.horse.strategy, this.course.distance);
    ...
    if (strategyMatches(this.horse.strategy, Strategy.Front) || this.pacer == null) {
      this.updatePositionKeep = noop as () => void;
    } else {
  ...
  initHills() {
    ...
    assert(isSortedByStart(this.course.slopes), 'slopes must be sorted by start location');
  ...
  updatePositionKeepNonNige() {
    if (this.pos >= this.posKeepEnd) {
      ...
      if (
        (this.pacer && this.pacer.pos - this.pos > this.posKeepEffectExitDistance) ||
  ...
  updatePhase() {
    ...
    if (this.pos >= this.nextPhaseTransition && this.phase < 2) {
      ++this.phase;
      this.nextPhaseTransition = phaseStart(this.course.distance, (this.phase + 1) as Phase);
    }
  }
  ...
  processSkillActivations() {
    for (let i = this.activeTargetSpeedSkills.length; --i >= 0; ) {
      ...
      if (s.durationTimer.t >= 0) {
        ...
        this.onSkillDeactivate(this, s.skillId, s.perspective ?? Perspective.Self);
      }
    }
  ...
  activateSkill(s: PendingSkill) {
    ...
    this.onSkillActivate(this, s.skillId, s.perspective ?? Perspective.Self);
  }

  doActivateRandomGold(ngolds: number) {
    const goldIndices = this.pendingSkills.reduce((acc: number[], skill, i) => {
      if (

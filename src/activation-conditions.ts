...
import {
  ...
  UniformRandomPolicy,
} from './activation-sample-policy';
import {
  assertIsDistanceType,
  assertIsPhase,
  CourseData,
  isSortedByStart,
  Phase,
...
import { assertIsStrategy, HorseParameters, Strategy, strategyMatches } from './horse-type';
import { RaceParameters } from './race-parameters';
import { DynamicCondition, RaceState } from './race-solver';
import { Region, RegionList } from './region';

// K as in SKI combinators
function kTrue(__: RaceState) {
  return true;
}
...
export class EqOperator {
  ...
  apply(regions: RegionList, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
    return withDefaultCond(this.condition.filterEq(regions, this.argument, course, horse, _extra));
  }
}
...
export class NeqOperator {
  ...
  apply(regions: RegionList, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
    return withDefaultCond(this.condition.filterNeq(regions, this.argument, course, horse, _extra));
  }
}
...
export class LtOperator {
  ...
  apply(regions: RegionList, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
    return withDefaultCond(this.condition.filterLt(regions, this.argument, course, horse, _extra));
  }
}
...
export class LteOperator {
  ...
  apply(regions: RegionList, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
    return withDefaultCond(this.condition.filterLte(regions, this.argument, course, horse, _extra));
  }
}
...
export class GtOperator {
  ...
  apply(regions: RegionList, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
    return withDefaultCond(this.condition.filterGt(regions, this.argument, course, horse, _extra));
  }
}
...
export class GteOperator {
  ...
  apply(regions: RegionList, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
    return withDefaultCond(this.condition.filterGte(regions, this.argument, course, horse, _extra));
  }
}
...
export class AndOperator {
  ...
  apply(regions: RegionList, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
    const [leftval, leftcond] = this.left.apply(regions, course, horse, _extra);
    const [rightval, rightcond] = this.right.apply(leftval, course, horse, _extra);
...
export class OrOperator {
  ...
  apply(regions: RegionList, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
    const [leftval, leftcond] = this.left.apply(regions, course, horse, _extra);
    const [rightval, rightcond] = this.right.apply(regions, course, horse, _extra);
...
function notSupported(_0: RegionList, _1: number, _2: CourseData, _3: HorseParameters, _extra: RaceParameters): never {
  assert(false, 'unsupported comparison');
  throw 0; // appease typescript
}

function noop(regions: RegionList, _1: number, _2: CourseData, _3: HorseParameters, _extra: RaceParameters) {
  return regions;
}
...
function valueFilter(getValue: (c: CourseData, h: HorseParameters, e: RaceParameters) => number) {
  return immediate({
    filterEq(regions: RegionList, value: number, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
      return getValue(course, horse, _extra) == value ? regions : new RegionList();
    },
    filterNeq(regions: RegionList, value: number, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
      return getValue(course, horse, _extra) != value ? regions : new RegionList();
    },
    filterLt(regions: RegionList, value: number, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
      return getValue(course, horse, _extra) < value ? regions : new RegionList();
    },
    filterLte(regions: RegionList, value: number, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
      return getValue(course, horse, _extra) <= value ? regions : new RegionList();
    },
    filterGt(regions: RegionList, value: number, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
      return getValue(course, horse, _extra) > value ? regions : new RegionList();
    },
    filterGte(regions: RegionList, value: number, course: CourseData, horse: HorseParameters, _extra: RaceParameters) {
      return getValue(course, horse, _extra) >= value ? regions : new RegionList();
    },
  });
}
...
function orderFilter(getPos: (arg: number, n?: number) => number) {
  return immediate({
    filterEq(regions: RegionList, arg: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      if (_extra.orderRange != null) {
        const pos = getPos(arg, _extra.numUmas);
        return pos >= _extra.orderRange[0] && pos <= _extra.orderRange[1] ? regions : new RegionList();
      }
      return regions;
    },
    filterNeq(regions: RegionList, arg: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      if (_extra.orderRange != null) {
        const pos = getPos(arg, _extra.numUmas);
        return pos < _extra.orderRange[0] || pos > _extra.orderRange[1] ? regions : new RegionList();
      }
    ...
    filterLt(regions: RegionList, arg: number, course: CourseData, __: HorseParameters, _extra: RaceParameters) {
      if (_extra.orderRange != null) {
        assert(1 <= _extra.orderRange[0] && _extra.orderRange[0] <= _extra.orderRange[1]);
        ...
        const end = new Region(phaseStart(course.distance, 2) + 100, course.distance);
        const pos = getPos(arg, _extra.numUmas ?? 18);
        return _extra.orderRange[0] < pos ? regions : regions.rmap((r) => r.intersect(end));
      }
      return regions;
    },
    filterLte(regions: RegionList, arg: number, course: CourseData, __: HorseParameters, _extra: RaceParameters) {
      if (_extra.orderRange != null) {
        assert(1 <= _extra.orderRange[0] && _extra.orderRange[0] <= _extra.orderRange[1]);
        const end = new Region(phaseStart(course.distance, 2) + 100, course.distance);
        const pos = getPos(arg, _extra.numUmas ?? 18);
    ...
    filterGt(regions: RegionList, arg: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      if (_extra.orderRange != null) {
        assert(_extra.orderRange[0] <= _extra.orderRange[1] && _extra.orderRange[1] <= (_extra.numUmas ?? 18));
        const pos = getPos(arg, _extra.numUmas ?? 18);
        return pos < _extra.orderRange[1] ? regions : new RegionList();
      }
      return regions;
    },
    filterGte(regions: RegionList, arg: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      if (_extra.orderRange != null) {
        assert(_extra.orderRange[0] <= _extra.orderRange[1] && _extra.orderRange[1] <= (_extra.numUmas ?? 18));
        const pos = getPos(arg, _extra.numUmas ?? 18);
        return pos <= _extra.orderRange[1] ? regions : new RegionList();
...
function orderInFilter(rate: number) {
  return immediate({
    filterEq(regions: RegionList, one: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      assert(one == 1, 'must be order_rate_inXX_continue==1');
      if (_extra.orderRange != null) {
        assert(1 <= _extra.orderRange[0] && _extra.orderRange[0] <= _extra.orderRange[1]);
        return _extra.orderRange[0] <= Math.round(rate * (_extra.numUmas ?? 18)) ? regions : new RegionList();
      }
      return regions;
    },
  });
...
function orderOutFilter(rate: number) {
  return immediate({
    filterEq(regions: RegionList, one: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      ...
      if (_extra.orderRange != null) {
        assert(_extra.orderRange[0] <= _extra.orderRange[1] && _extra.orderRange[1] <= (_extra.numUmas ?? 18));
        return Math.round(rate * (_extra.numUmas ?? 18)) <= _extra.orderRange[1] ? regions : new RegionList();
      }
      return regions;
    },
  });
...
export const Conditions: { [cond: string]: Condition } = {
  accumulatetime: immediate({
    filterGte(regions: RegionList, t: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      return [regions, (s: RaceState) => s.accumulatetime.t >= t] as [RegionList, DynamicCondition];
    },
  }),
  activate_count_all: immediate({
    filterLte(regions: RegionList, n: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      return [regions, (s: RaceState) => s.activateCount.reduce((a, b) => a + b) <= n] as [
        RegionList,
        DynamicCondition,
      ];
    },
    filterGte(regions: RegionList, n: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      return [regions, (s: RaceState) => s.activateCount.reduce((a, b) => a + b) >= n] as [
        RegionList,
        DynamicCondition,
      ];
  ...
  all_corner_random: {
    ...
    filterEq(regions: RegionList, one: number, course: CourseData, __: HorseParameters, _extra: RaceParameters) {
  ...
  base_power: valueFilter((__: CourseData, horse: HorseParameters, _extra: RaceParameters) => horse.power),
  base_speed: valueFilter((__: CourseData, horse: HorseParameters, _extra: RaceParameters) => horse.speed),
  base_stamina: valueFilter((__: CourseData, horse: HorseParameters, _extra: RaceParameters) => horse.stamina),
  base_guts: valueFilter((__: CourseData, horse: HorseParameters, _extra: RaceParameters) => horse.guts),
  base_wiz: valueFilter((__: CourseData, horse: HorseParameters, _extra: RaceParameters) => horse.wisdom),
  ...
  change_order_up_end_after: erlangRandom(3, 2.0, {
    filterGte(regions: RegionList, _0: number, course: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      const bounds = new Region(phaseStart(course.distance, 2), course.distance);
      return regions.rmap((r) => r.intersect(bounds));
    },
  }),
  change_order_up_finalcorner_after: erlangRandom(3, 2.0, {
    filterGte(regions: RegionList, _0: number, course: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      assert(isSortedByStart(course.corners), 'course corners must be sorted by start');
  ...
  change_order_up_middle: erlangRandom(3, 2.0, {
    filterGte(regions: RegionList, _0: number, course: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      const bounds = new Region(phaseStart(course.distance, 1), phaseEnd(course.distance, 1));
      return regions.rmap((r) => r.intersect(bounds));
    },
  }),
  compete_fight_count: uniformRandom({
    filterGt(regions: RegionList, _0: number, course: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      assert(isSortedByStart(course.straights), 'course straights must be sorted by start');
      const lastStraight = course.straights[course.straights.length - 1];
      return regions.rmap((r) => r.intersect(lastStraight));
    },
  }),
  corner: immediate({
    filterEq(regions: RegionList, cornerNum: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      assert(isSortedByStart(course.corners), 'course corners must be sorted by start');
      if (cornerNum == 0) {
    ...
    filterNeq(regions: RegionList, cornerNum: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      assert(cornerNum == 0, 'only supports corner!=0');
      const corners = course.corners.map((c) => new Region(c.start, c.start + c.length));
      return regions.rmap((r) => corners.map((c) => r.intersect(c)));
    },
  }),
  corner_count: valueFilter((course: CourseData, __: HorseParameters, _extra: RaceParameters) => course.corners.length),
  ...
  course_distance: valueFilter((course: CourseData, __: HorseParameters, _extra: RaceParameters) => course.distance),
  ...
  distance_rate: immediate({
    filterLte(regions: RegionList, rate: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      const bounds = new Region(0, (course.distance * rate) / 100);
      return regions.rmap((r) => r.intersect(bounds));
    },
    filterGte(regions: RegionList, rate: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      const bounds = new Region((course.distance * rate) / 100, course.distance);
      return regions.rmap((r) => r.intersect(bounds));
    },
  }),
  distance_rate_after_random: random({
    filterEq(regions: RegionList, rate: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      const bounds = new Region((course.distance * rate) / 100, course.distance);
      return regions.rmap((r) => r.intersect(bounds));
    },
  }),
  distance_type: immediate({
    filterEq(
      ...
      assertIsDistanceType(distanceType);
  ...
  down_slope_random: random({
    filterEq(regions: RegionList, one: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
  ...
  grade: valueFilter((_0: CourseData, _1: HorseParameters, _extra: RaceParameters) => _extra.grade),
  ground_condition: valueFilter(
    (_0: CourseData, _1: HorseParameters, _extra: RaceParameters) => _extra.groundCondition,
  ),
  ground_type: valueFilter((course: CourseData, __: HorseParameters, _extra: RaceParameters) => course.surface),
  hp_per: immediate({
    filterLte(regions: RegionList, hpPer: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      hpPer /= 100;
      return [regions, (s: RaceState) => s.hp.hpRatioRemaining() <= hpPer] as [RegionList, DynamicCondition];
    },
    filterGte(regions: RegionList, hpPer: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      hpPer /= 100;
      return [regions, (s: RaceState) => s.hp.hpRatioRemaining() >= hpPer] as [RegionList, DynamicCondition];
    },
  }),
  ...
  is_activate_other_skill_detail: immediate({
    filterEq(regions: RegionList, one: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      ...
      return [regions, (s: RaceState) => s.usedSkills.has(_extra.skillId)] as [RegionList, DynamicCondition];
    },
  }),
  is_basis_distance: immediate({
    filterEq(regions: RegionList, flag: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
  ...
  is_badstart: immediate({
    filterEq(regions: RegionList, flag: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
  ...
  is_dirtgrade: immediate({
    ...
    filterNeq(regions: RegionList, flag: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      assert(flag == 1, 'must be is_dirtgrade!=1');
      return [10101, 10103, 10104, 10105].indexOf(course.raceTrackId) == -1 ? regions : new RegionList();
    },
  }),
  ...
  is_hp_empty_onetime: immediate({
    filterEq(regions: RegionList, one: number, _course: CourseData, _: HorseParameters, _extra: RaceParameters) {
  ...
  is_used_skill_id: immediate({
    filterEq(regions: RegionList, skillId: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      return [regions, (s: RaceState) => s.usedSkills.has('' + skillId)] as [RegionList, DynamicCondition];
    },
  }),
  lane_type: noopImmediate,
  lastspurt: immediate({
    filterEq(regions: RegionList, case_: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
  ...
  motivation: valueFilter((_0: CourseData, _1: HorseParameters, _extra: RaceParameters) => _extra.mood + 3), // go from -2 to 2 to 1-5 scale
  near_count: noopErlangRandom(3, 2.0),
  order: orderFilter((pos: number, __: number) => pos),
  ...
  phase: {
    samplePolicy: ImmediatePolicy,
    filterEq(regions: RegionList, phase: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      assertIsPhase(phase);
      ...
      const fudge =
        ['100591', '900591', '110261', '910261', '110191', '910191', '120451', '920451', '101502121'].indexOf(
          _extra.skillId,
        ) > -1
          ? 10
          : 0;
      const bounds = new Region(phaseStart(course.distance, phase), phaseEnd(course.distance, phase) + fudge);
      return regions.rmap((r) => r.intersect(bounds));
    },
    filterNeq: notSupported,
    filterLt(regions: RegionList, phase: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      assertIsPhase(phase);
      assert(phase > 0, 'phase == 0');
      const bounds = new Region(0, phaseStart(course.distance, phase));
      return regions.rmap((r) => r.intersect(bounds));
    },
    filterLte(regions: RegionList, phase: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      assertIsPhase(phase);
      const bounds = new Region(0, phaseEnd(course.distance, phase));
      return regions.rmap((r) => r.intersect(bounds));
    },
    filterGt(regions: RegionList, phase: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      assertIsPhase(phase);
      assert(phase < 3, 'phase > 2');
      const bounds = new Region(phaseStart(course.distance, (phase + 1) as Phase), course.distance);
      return regions.rmap((r) => r.intersect(bounds));
    },
    filterGte(regions: RegionList, phase: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      assertIsPhase(phase);
      const bounds = new Region(phaseStart(course.distance, phase), course.distance);
      return regions.rmap((r) => r.intersect(bounds));
    },
  },
  phase_corner_random: random({
    filterEq(regions: RegionList, phase: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      assertIsPhase(phase);
      const phaseS = phaseStart(course.distance, phase);
      const phaseE = phaseEnd(course.distance, phase);
      const corners = course.corners
        .filter(
          (c) =>
            (c.start >= phaseS && c.start < phaseE) || (c.start + c.length >= phaseS && c.start + c.length < phaseE),
        )
        .map((c) => new Region(Math.max(c.start, phaseS), Math.min(c.start + c.length, phaseE)));
      return regions.rmap((r) => corners.map((c) => r.intersect(c)));
    },
  }),
  phase_firsthalf: immediate({
    filterEq(regions: RegionList, phase: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      assertIsPhase(phase);
      const start = phaseStart(course.distance, phase);
      const end = phaseEnd(course.distance, phase);
  ...
  phase_random: random({
    filterEq(regions: RegionList, phase: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      assertIsPhase(phase);
      const bounds = new Region(phaseStart(course.distance, phase), phaseEnd(course.distance, phase));
      return regions.rmap((r) => r.intersect(bounds));
    },
  }),
  phase_straight_random: {
    ...
    filterEq(regions: RegionList, phase: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      assertIsPhase(phase);
      const phaseBounds = new Region(phaseStart(course.distance, phase), phaseEnd(course.distance, phase));
  ...
  post_number: (function () {
    ...
    return immediate({
      filterEq(regions: RegionList, post: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
        return [regions, (s: RaceState) => gateBlock(s, _extra.numUmas || 9) == post] as [RegionList, DynamicCondition];
      },
      filterLte(regions: RegionList, post: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
        return [regions, (s: RaceState) => gateBlock(s, _extra.numUmas || 9) <= post] as [RegionList, DynamicCondition];
      },
      filterGte(regions: RegionList, post: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
        return [regions, (s: RaceState) => gateBlock(s, _extra.numUmas || 9) >= post] as [RegionList, DynamicCondition];
      },
    });
  })(),
  random_lot: immediate({
    filterEq(regions: RegionList, lot: number, _0: CourseData, _1: HorseParameters, _extra: RaceParameters) {
      return [regions, (s: RaceState) => s.randomLot < lot] as [RegionList, DynamicCondition];
    },
  }),
  remain_distance: immediate({
    filterEq(regions: RegionList, remain: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      const bounds = new Region(course.distance - remain, course.distance - remain + 1);
      return regions.rmap((r) => r.intersect(bounds));
    },
    filterLte(regions: RegionList, remain: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      const bounds = new Region(course.distance - remain, course.distance);
      return regions.rmap((r) => r.intersect(bounds));
    },
    filterGte(regions: RegionList, remain: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      const bounds = new Region(0, course.distance - remain);
      return regions.rmap((r) => r.intersect(bounds));
    },
  }),
  rotation: valueFilter((course: CourseData, __: HorseParameters, _extra: RaceParameters) => course.turn),
  running_style: immediate({
    filterEq(regions: RegionList, strategy: number, _: CourseData, horse: HorseParameters, _extra: RaceParameters) {
      assertIsStrategy(strategy);
      if (strategyMatches(horse.strategy, strategy)) {
        return regions;
      } else {
        return new RegionList();
      }
  ...
  running_style_count_nige_otherself: valueFilter(
    (_: CourseData, horse: HorseParameters, _extra: RaceParameters) => +strategyMatches(horse.strategy, Strategy.Front),
  ),
  running_style_count_senko_otherself: valueFilter(
    (_: CourseData, horse: HorseParameters, _extra: RaceParameters) => +strategyMatches(horse.strategy, Strategy.Pace),
  ),
  running_style_count_sashi_otherself: valueFilter(
    (_: CourseData, horse: HorseParameters, _extra: RaceParameters) => +strategyMatches(horse.strategy, Strategy.Late),
  ),
  running_style_count_oikomi_otherself: valueFilter(
    (_: CourseData, horse: HorseParameters, _extra: RaceParameters) => +strategyMatches(horse.strategy, Strategy.End),
  ),
  ...
  season: valueFilter((_0: CourseData, _1: HorseParameters, _extra: RaceParameters) => _extra.season),
  slope: immediate({
    filterEq(regions: RegionList, slopeType: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
      ...
      assert(isSortedByStart(course.slopes), 'course slopes must be sorted by slope start');
      let lastEnd = 0;
  ...
  straight_front_type: immediate({
    filterEq(regions: RegionList, frontType: number, course: CourseData, _: HorseParameters, _extra: RaceParameters) {
  ...
  time: valueFilter((_0: CourseData, _1: HorseParameters, _extra: RaceParameters) => _extra.time),
  track_id: valueFilter((course: CourseData, _: HorseParameters, _extra: RaceParameters) => course.raceTrackId),
  ...
  weather: valueFilter((_0: CourseData, _1: HorseParameters, _extra: RaceParameters) => _extra.weather),
};

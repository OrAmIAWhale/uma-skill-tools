...
import { ErlangRandomPolicy } from '../activation-sample-policy';
import { getParser } from '../condition-parser';
import { Perspective, RaceSolver, SkillRarity, SkillType } from '../race-solver';
import { RaceSolverBuilder, buildBaseStats, buildSkillData } from '../race-solver-builder';
...
program
  ...
  .addOption(
    new Option('--sheet <sheet ID>', 'use the Google Sheets API').implies({
      csv: false,
    }),
  )
  .addOption(
    new Option('--api-key <path to key>', 'path to service account private key (JSON)').env(
      'GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY',
    ),
  )
  .addOption(
    new Option('-N, --nsamples <N>', 'number of random samples to use for skills with random conditions')
      .default(500)
      .argParser((x) => parseInt(x, 10)),
  )
  .addOption(
    new Option('-s, --strategy <strategy>', 'strategy to test skills for').choices(['front', 'pace', 'late', 'end']),
  )
  .addOption(
  ...
  .addOption(
    new Option('-D, --distance-aptitude <letter>', 'distance aptitude')
      .choices(['S', 'A', 'B', 'C', 'D', 'E', 'F', 'G'])
      .default('S'),
  )
  .addOption(
    new Option('-G, --surface-aptitude <letter>', 'surface aptitude')
      .choices(['S', 'A', 'B', 'C', 'D', 'E', 'F', 'G'])
      .default('A'),
  )
  .addOption(
    new Option('-S, --strategy-aptitude <letter>', 'strategy aptitude')
      .choices(['S', 'A', 'B', 'C', 'D', 'E', 'F', 'G'])
      .default('A'),
  )
...
function getBuilder(strategy: string) {
  ...
  const builder = new RaceSolverBuilder(options.nsamples)
    ...
    .popularity(
      !('popularity' in cmdef) ? 1 : cmdef.popularity < 0 ? cmdef.totalUmas + 1 + cmdef.popularity : cmdef.popularity,
    )
    .order(
      cmdef.strategyPositions[strategy][0],
      cmdef.strategyPositions[strategy][cmdef.strategyPositions[strategy].length - 1],
    )
...
function displayRow(r) {
  ...
  cols.push.apply(
    cols,
    r.thresholds.map((t) => t.toFixed(2)),
  );
  return cols;
}
...
const COL_NAME_SETS = Object.freeze({
  greens: [
    ['バ身', 'スキル'],
    ['Bashin gain', 'Skill name'],
  ],
  pinks_golds: [
    ['平均', 'スキル', '最小', '最大', '中央', '<0.25', '≥0.75', '≥1.50'],
    ['Mean', 'Skill name', 'Min', 'Max', 'Median', '<0.25', '≥0.75', '≥1.50'],
  ],
  whites_uniques: [
    ['平均', 'スキル', '最小', '最大', '中央', '<0.15', '≥0.33', '≥0.67'],
    ['Mean', 'Skill name', 'Min', 'Max', 'Median', '<0.15', '≥0.33', '≥0.67'],
  ],
});
...
const THRESHOLDS_PINK_GOLD = [-0.25, 0.75, 1.5];
...
if (options.csv) {
  ...
  const strategies = options.strategy ? [options.strategy] : ['front', 'pace', 'late', 'end'];
  strategies.forEach(async (strategy) => {
    ...
    await sheet.setHeaderRow(
      COL_NAME_SETS.whites_uniques[lang],
      greenRows.length + pinkRows.length + goldRows.length + 7,
    );
    await sheet.addRows(whiteRows.map(displayRow));

    await sheet.setHeaderRow(
      COL_NAME_SETS.whites_uniques[lang],
      greenRows.length + pinkRows.length + goldRows.length + whiteRows.length + 9,
    );
    ...
    sectionOffsets.push(
      greenRows.length + pinkRows.length + goldRows.length + whiteRows.length + uniqueRows.length + 10,
    );
    const valueRanges: {
        sheetId: number;
    ...
    for (let i = 0; i < sectionOffsets.length - 1; ++i) {
      const rfirst = sectionOffsets[i] + 1,
        rlast = sectionOffsets[i + 1] - 2;
      valueRanges.push({
        sheetId: sheet.sheetId,
        startRowIndex: rfirst,
        endRowIndex: rlast + 1,
        startColumnIndex: 0,
        endColumnIndex: 1,
      });
      ...
      for (let r = rfirst; r <= rlast; ++r) {
        ...
        cols.forEach(
          (c) =>
            (sheet.getCell(r, c).numberFormat = {
              type: 'NUMBER',
              pattern: '0.00',
            }),
        );
      ...
      if (lang == 1) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await sheet.updateDimensionProperties('ROWS', { pixelSize: 40 } as any, {
          startIndex: rfirst,
          endIndex: rlast + 1,
        });
      }
    ...
    sheet.getCellByA1('E4').value = cmdef.presupposedSkills[strategy]
      .map((id) => skillnames[id][0])
      .join(STRINGS[','][0]);
    if (lang != 0) {
      // non-jp
      sheet.getCellByA1('E4').note = cmdef.presupposedSkills[strategy]
        .map((id) => skillnames[id][lang])
        .join(STRINGS[','][lang]);

...
import { RaceSolver } from '../race-solver';
import { Rule30CARng } from '../random';
import { PacerProvider, SkillData, ToolCLI } from './cli';
...
cli.options((program) => {
  program
    ...
    .addOption(
      new Option('-C, --configuration <confstring>').conflicts('seed').argParser((s) => Buffer.from(s, 'base64')),
    );
});
cli.run(
  (
    ...
    } else if ('configuration' in cliOptions && cliOptions.configuration) {
    ...
    const s = new RaceSolver({
      ...
      skills,
      hp: NoopHpPolicy,
      pacer: getPacer(pacerRng) || undefined,
      rng: solverRng,

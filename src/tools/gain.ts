...
import { ImmediatePolicy, RandomPolicy } from '../activation-sample-policy';
import { CourseData } from '../course-data';
import { HorseParameters } from '../horse-type';
import { NoopHpPolicy } from '../hp-policy';
import { PendingSkill, RaceSolver, SkillEffect, SkillRarity } from '../race-solver';
import { Rule30CARng } from '../random';
import { Region } from '../region';
import { PacerProvider, parseAptitude, SkillData, ToolCLI } from './cli';
...
cli.options((program) => {
  program
    ...
    .addOption(
      new Option(
        '-D, --distance-aptitude <letter>',
        'compare with a different distance aptitude from the value in the horse definition',
      )
    ...
    .addOption(
      new Option(
        '-S, --surface-aptitude <letter>',
        'compare with a different surface aptitude from the value in the horse definition',
      )
        .choices(['S', 'A', 'B', 'C', 'D', 'E', 'F', 'G'])
    ...
    .addOption(
      new Option(
        '--thresholds <cutoffs>',
        'comma-separated list of values; print the percentage of the time they are exceeded',
      )
...
cli.run(
  (
    ...
    cliOptions: import('./cli').CliOptions,
  ) => {
    const nsamples = cliOptions.nsamples || 500;
    const triggers: Region[][] = [];
    const seed =
      ('seed' in cliOptions && cliOptions.seed ? cliOptions.seed : Math.floor(Math.random() * (-1 >>> 0))) >>> 0;
    ...
    const dt = cliOptions.timestep || 1 / 60;
    ...
    if (cliOptions.distanceAptitude != undefined) {
      testHorse = Object.freeze(
        Object.assign({}, testHorse, {
          distanceAptitude: cliOptions.distanceAptitude,
        }),
      );
    }
    if (cliOptions.surfaceAptitude != undefined) {
      testHorse = Object.freeze(
        Object.assign({}, testHorse, {
          surfaceAptitude: cliOptions.surfaceAptitude,
        }),
      );
    }
    ...
    for (let i = 0; i < nsamples; ++i) {
      const seedhi = solverRng1.hi,
        seedlo = solverRng1.lo,
        pacerseedhi = pacerRng1.hi,
        pacerseedlo = pacerRng1.lo;
      const skillCheckRolls: number[] = [];
      ...
      const s2 = new RaceSolver({
        horse,
        course,
        hp: NoopHpPolicy,
        skills: skills2,
        pacer: getPacer(pacerRng2),
        rng: solverRng2,
      });
      ...
      if (
        solverRng1.hi != solverRng2.hi ||
        solverRng1.lo != solverRng2.lo ||
        pacerRng1.hi != pacerRng2.hi ||
        pacerRng1.lo != pacerRng2.lo
      ) {
    ...
    if (cliOptions.csv) {
      ...
      cliOptions.thresholds.forEach((n) => {
        console.log(
          '≥' + n.toFixed(2) + ' | ' + ((gain.reduce((a, b) => a + +(b >= n), 0) / gain.length) * 100).toFixed(2) + '%',
        );
      });
      ...
      // Node.js Buffer import for compatibility

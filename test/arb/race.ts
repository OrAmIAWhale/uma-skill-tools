...
const skillids = Object.freeze(
  Object.keys(skills).filter((id) => {
    ...
    const b = new build.RaceSolverBuilder(1)
      .course(10101)
      .horse({
        speed: 1000,
        stamina: 1000,
...
export function GroundCondition() {
  return fc
    .constantFrom(
      rparams.GroundCondition.Good,
      rparams.GroundCondition.Yielding,
      rparams.GroundCondition.Soft,
      rparams.GroundCondition.Heavy,
    )
    .noBias();
}
...
export function Weather() {
  return fc
    .constantFrom(rparams.Weather.Sunny, rparams.Weather.Cloudy, rparams.Weather.Rainy, rparams.Weather.Snowy)
    .noBias();
}
...
export function Season() {
  return fc
    .constantFrom(
      rparams.Season.Spring,
      rparams.Season.Summer,
      rparams.Season.Autumn,
      rparams.Season.Winter,
      rparams.Season.Sakura,
    )
    .noBias();
}
...
export function Stat() {
  return fc.integer({ min: 100, max: 1200 });
}

export function Strategy() {
  return fc.constantFrom('Front', 'Pace', 'Late', 'End', 'Escape').noBias();
}
...
export function SkillList(max: number = 30) {
  // If no valid skills are available, return empty array to avoid constantFrom error
...
export function Race({
  ...
  maxPreSkills = 30,
  maxSut = 30,
}: { maxSamples?: number; maxPreSkills?: number; maxSut?: number } = {}) {
  return fc.record({

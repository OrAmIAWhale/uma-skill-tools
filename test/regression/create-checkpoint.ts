...
program
  .addOption(
    new Option('-t, --tests <number>', 'number of test cases to generate')
      .default(10000)
      .argParser((n) => parseInt(n, 10)),
  )
  .addOption(
    new Option('--timestep <dt>', 'integration timestep in seconds')
      .default(1 / 15, '1/15')
      .argParser((ts) => ts.split('/').reduceRight((a, b) => +b / +a, 1.0)),
  )
  .addOption(
    new Option('--seed <number>', 'seed for random generator')
...
const results: { params: any; result: any; timestep: number }[] = [];
fc.sample(arb.Race(), options.tests).forEach((params) => {
  ...
  const result: { err: boolean; gain: number[] } = { err: false, gain: [] };
...
const outfile = path.join(
  ...
  'checkpoints',
  date + '.' + rev.slice(0, 7) + '.' + options.seed + '.json',
);

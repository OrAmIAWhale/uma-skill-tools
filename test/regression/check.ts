...
import { Rule30CARng } from '../../src/random';
  ...
  this._assert(almostEqual(a, b), {
    message:
      ...
      'should be closer than ' +
        Math.max(Epsilon * (Math.abs(a) + Math.abs(b)), Number.EPSILON) +
        ' (actual difference: ' +
        Math.abs(a - b) +
        ')',
...
function getLatestCheckpoint() {
  ...
  return fs
    ...
    .map(
      (f) =>
        [path.join(dir, f), Date.parse(f.split('.', 1)[0].replace(/(\d{4})(\d{2})(\d{2})/, '$1-$2-$3'))] as [
          string,
          number,
        ],
    )
...
program
  ...
  .addOption(
    new Option('--seed <number>', 'seed to use for random shuffle with --fast')
      .implies({ fast: true })
...
test('should give results similar to the checkpoint', (t) => {
  ...
  const failures: { params: any; caseIdx: number; sampleIdx: number; expected: number; actual: number }[] = [];
      ...
      try {
        ...
        if (almostEqual(s1.pos - s2.pos, testCase.result.gain[i])) {
          ...
          failures.push({
            ...
            caseIdx: (t as any).assertCount - 1,
            sampleIdx: i,
            expected: testCase.result.gain[i],
            actual: s1.pos - s2.pos,
          });
        }
      } catch {
        err = true;
  ...
  if (failures.length > 0) {
    ...
    t.comment(
      'wrote ' + failures.length + ' failure' + (failures.length == 1 ? '' : 's') + ' to ' + options.failureLog,
    );
  }
